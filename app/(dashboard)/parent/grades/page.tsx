...
export default function ParentGradesPage() {
  const { user } = useAuth();
  const [grades, setGrades] = useState<(Grade & { class?: ClassInfo; studentName?: string })[]>([]);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-8">Children's Grades</h1>
            ...
            <tbody>
              {grades.map((grade) => (
                <tr key={grade.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="py-4 px-6 font-medium">{grade.studentName || 'Unknown'}</td>
